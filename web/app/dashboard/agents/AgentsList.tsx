// web/app/dashboard/agents/AgentsList.tsx
'use client';
...
import type { ApiKey, AgentStateName, Order } from '../_lib/types';
...
export default function AgentsList({ initialAgents, initialOrders }: { initialAgents: ApiKey[], initialOrders: Order[] }) {
  const { walletBalances, agents: liveAgents, orders: liveOrders } = useDashboard();
  ...
  // Hybrid strategy: use server props for instant load, but sync with 
  // live provider state for real-time updates after creation/mutation.
  const agents = liveAgents.length > 0 ? liveAgents : initialAgents;
