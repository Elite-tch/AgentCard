...
export const CHANGELOG_ENTRIES: ChangelogEntry[] = [
  ...
  },
  {
    ...
    tags: ['fix', 'api'],
    body: 'Two bugs in the SDK\'s Soroban submit path were combining to leave agents polling forever on orders whose payment never actually landed. (1) FAILED statuses from getTransaction were swallowed by a catch-block intended for XDR version errors, so the poll loop ran to the 120s deadline. (2) The deadline-timeout branch attached a txHash to its error unconditionally, which purchaseCardOWS treated as "envelope may still land" — so transactions that had already failed on-chain or never made it into a ledger still triggered the waitForCard fall-through. Fixed by re-raising terminal throws out of the poll loop and only attaching txHash when Horizon itself is unreachable. Package exports now declare both `import` and `require` so CommonJS consumers don\'t trip on ERR_PACKAGE_PATH_NOT_EXPORTED. Upgrade with `npx -y agentcard@latest` or `npm i agentcard@latest`.',
  },
  {
    date: '2026-04-14',
    title: 'New post: claim codes — credentials that never touch the transcript',
    tags: ['feature'],
    body: 'Security-architecture post on why AgentCard onboards agents with single-use claim codes instead of raw API keys. The failure mode (keys in LLM chat transcripts), the three options we considered (OAuth, env-only instructions, one-time exchange tokens), why we picked the last one, and what else building the primitive unlocked.',
  },
  {
    date: '2026-04-14',
    title: 'New post: why SSE beats polling for agent-facing APIs',
    tags: ['feature'],
    body: 'Why the AgentCard SDK defaults to Server-Sent Events for order state, why we kept polling as an automatic fallback, and the operational details (keepalive interval, nginx buffering, terminal close) that make SSE reliable in practice when your clients are long-lived processes instead of browsers.',
  },
  {
    date: '2026-04-14',
    title: 'New post: non-custodial card issuance on Soroban',
    tags: ['feature'],
    body: "Architectural walk-through of the AgentCard receiver contract, the Soroban event watcher, and the refund story. Why agents pay a contract we can't drain, what we gave up on the latency side, and why the trade was worth it for a payment platform aimed at autonomous agents.",
  },
  {
    date: '2026-04-14',
    title: 'First blog post: anatomy of a AgentCard order',
    tags: ['feature'],
  ...
  },
  {
    ...
    version: '1.2.0',
    tags: ['feature'],
    body: 'Docs page rewritten onto the Fraunces/IBM Plex type system with editorial section scaffolding. New favicon, AgentCard casing swept across every user-visible surface, notification tray with empty state, login form now submits on Enter.',
  },
  {
    date: '2026-04-13',
  ...
  },
  {
    ...
    version: '1.0.0',
    tags: ['api', 'security'],
    body: 'Non-custodial payment flow: agents now sign and submit Soroban contract invocations directly to the receiver contract. AgentCard proxies the 402 response and observes on-chain events. No funds held in intermediate custody.',
  },
  {
    date: '2026-04-05',
