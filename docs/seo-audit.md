# Agentcard SEO audit — 2026-04-14
...
Root layout defines `title: { default, template: '%s — Agentcard' }`. Pages
export `metadata.title: 'Pricing'` and Next.js renders
`<title>Pricing — Agentcard</title>`. No page repeats the brand in its own
title string — avoids the `Pricing — Agentcard — Agentcard` bug.
...
The image itself is referenced as `https://agentcard.com/opengraph-image` —
...
11. **Status page** at `status.agentcard.com`. Referenced from Terms but
...
12. **Security.txt.** Add `/.well-known/security.txt` pointing at
    `security@agentcard.com` so researchers find us without clicking through
    to /security.
...
- 🟢 **BreadcrumbList structured data** — live on `/docs/quickstart` and
  ...
  shows `Agentcard › Docs › Quickstart` in the SERP.
...
- 🟢 **Status page (`/status`)** — closed the dangling reference from
  the Terms page to `status.agentcard.com`. Component-by-component
...
- ⚠️ Status page at `status.agentcard.com` (the real dashboard, not the
  Agentcard-hosted summary on `/status`).
